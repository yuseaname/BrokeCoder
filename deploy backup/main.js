...
function recalcResourceCaps() {
  ...
  if (!player.resources.focusEnergy || typeof player.resources.focusEnergy !== 'object') {
    player.resources.focusEnergy = { current: START_RESOURCES.focusEnergy, max: RESOURCE_CONFIG.focusEnergy.max };
  }
  if (!player.resources.nerveEnergy || typeof player.resources.nerveEnergy !== 'object') {
    player.resources.nerveEnergy = { current: START_RESOURCES.nerveEnergy, max: RESOURCE_CONFIG.nerveEnergy.max };
  }
  if (!player.resources.physicalEnergy || typeof player.resources.physicalEnergy !== 'object') {
    player.resources.physicalEnergy = { current: START_RESOURCES.physicalEnergy, max: RESOURCE_CONFIG.physicalEnergy.max };
  }
  ...
  player.resources.focusEnergy.max = RESOURCE_CONFIG.focusEnergy.max + (focus - 1);
  player.resources.nerveEnergy.max = RESOURCE_CONFIG.nerveEnergy.max + Math.floor((streetsmarts - 1) / 1.5);
  player.resources.physicalEnergy.max = RESOURCE_CONFIG.physicalEnergy.max + (fitness - 1);
